...
2. GET routes: filters (username, userage), pagination (limit, skip), sorting (sort_by, sort_order)
3. POST/PUT/DELETE routes protected by API key authentication
4. Input validation: username length, positive age
5. ObjectId safety: prevents crashes on invalid IDs
6. _id serialization: returns _id as string
7. HTTP status codes: 200, 201, 400, 401, 404
8. Optional total count returned for pagination
'''

# ----- Project Imports -----
from fastapi import FastAPI, APIRouter, status, HTTPException, Query, Depends
...
def api_key_auth(api_key: str = Query(..., description="API Key to authorize requests")):
...
@user_router.post("/create", status_code=status.HTTP_201_CREATED, response_model=ResponseModel, dependencies=[Depends(api_key_auth)])
...
# PUT update existing user (protected)
@user_router.put("/{user_id}", status_code=status.HTTP_200_OK, response_model=ResponseModel, dependencies=[Depends(api_key_auth)])
...
@user_router.delete("/{user_id}", status_code=status.HTTP_200_OK, response_model=ResponseModel, dependencies=[Depends(api_key_auth)])
