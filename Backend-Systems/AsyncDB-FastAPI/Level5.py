...
'''
1. Added user roles (admin or user)
2. Admin-only routes:
   - Create users
   - Delete users
   - View all users
3. Regular users can only update or view their own profile
4. JWT carries role information
5. Secure route access using require_admin and current_user
'''
...
# ----- App Settings -----
app = FastAPI(title="Pymongo_Async_App", version="4.0.0")
...
async def create_user(body: UserCreateModel, current_user: dict = Depends(get_current_user)):
    require_admin(current_user)  # Only admin can create users
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    access_token = create_access_token(data={"sub": str(user["_id"]), "role": user.get("role")}, expires_delta=timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES))
